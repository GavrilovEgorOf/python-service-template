...
async def test_auth_required_when_enabled(
    ...
    authed = await client.get("/api/v1/items", headers={"X-API-Key": "dev-api-key-change-me"})
    assert authed.status_code == 200


@pytest.mark.asyncio
async def test_bearer_auth(client: AsyncClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("app.core.config.settings.auth_disabled", False)
